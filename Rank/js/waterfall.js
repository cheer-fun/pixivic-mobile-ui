;(function () {
    ...
    WaterFull.prototype = {
        ...
        create: function (dataArr) {
            ...
            this.createSecond(dataArr).map(el => {
                this.$listNode.appendChild(el);
            });
            this.secondeReflows();
        ...
        createSecond: function (dataArr) {
            var _this = this,
                result = dataArr.map(function (item, index) {
                    var imgUrl = item.meta_pages.length > 0 ? item.meta_pages[0].image_urls.original:item.meta_single_page.original_image_url;
                    var height = _this._countRate(item[_this.opts.widthField], item[_this.opts.heightField]) * 47 + '%';
                    var a = document.createElement('a');
                    a.classList.add('image');
                    a.setAttribute('rel', `https://www.pixiv.net/member_illust.php?mode=medium&illust_id=${item.id}`);
                    a.innerHTML = `<div class="wf-item wf-item-2" style="padding-bottom: ${height};background-image: url(${imgUrl});"></div>`
                    a.pages = item.meta_pages.length > 0 ?
                        item.meta_pages.map(e => e.image_urls.original) : [imgUrl];
                    return a;
                });
            return result;
        },
